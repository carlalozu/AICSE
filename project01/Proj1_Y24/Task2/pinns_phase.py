...
class Pinns:
    ...
    def add_spatial_boundary_points_charging(self):
        ...
        output_sb_0 = torch.zeros((self.n_sb, 1)) + self.T_hot
        # Zero Neumann boundary condition
        output_sb_L = torch.zeros((self.n_sb, 1))
        return output_sb_0, output_sb_L

    def add_spatial_boundary_points_discharging(self):
        """Boundary points for discharging phase"""
        # Zero Neumann boundary condition
        output_sb_0 = torch.zeros((self.n_sb, 1))
        # Dirichlet boundary condition
        output_sb_L = torch.zeros((self.n_sb, 1)) + self.T_cold
        return output_sb_0, output_sb_L
    ...
    def compute_pde_residual(self, input_int):
        ...
        assert residual.shape[0] == self.n_int
...
class PinnLaterPhase(Pinns):
    """Derived class to solve the inverse problem for a later phase"""
    ...
    def initial_condition(self, x):
        """Initial condition to solve the equation based on output from previous
        phase"""
