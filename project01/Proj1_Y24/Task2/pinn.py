...
class Pinns:
    ...
    def __init__(self, n_int_, n_sb_, n_tb_, t0, tf, alpha_f, h_f,
        ...
        ########################################################
        max_iter = 10000
        self.optimizer = torch.optim.LBFGS(list(self.approximate_solution.parameters()) + list(self.approximate_coefficient.parameters()),
                              lr=float(0.5),
                              max_iter=max_iter,
                              max_eval=50000,
                              history_size=150,
    ...
    def add_spatial_boundary_points(self):
        ...
        output_sb_0 = torch.zeros((input_sb.shape[0], 1)) + self.T_hot
    ...
    def apply_boundary_conditions(self, input_sb):
        ...
        # Differentiate u_xL with input_sb
        u_xL_x = torch.autograd.grad(u_xL.sum(), input_sb,
                                     create_graph=True)[0][self.n_sb:, 1]
        u_xL_x = u_xL_x.reshape(-1, 1)
        # Concat
        u_bound = torch.cat([u_x0, u_xL_x], 0)
    ...
    def compute_pde_residual(self, input_int):
        ...
        residual = grad_u_t + self.u_f*grad_u_x - self.alpha_f*grad_u_xx + self.h_f*(u - k)
